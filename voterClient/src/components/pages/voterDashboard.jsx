...
import { User, MapPin, ShieldCheck, CheckCircle2, AlertTriangle, Vote, BarChart3, Gavel, Image as ImageIcon } from 'lucide-react';
...
export default function VoterDashboard() {
  ...
  // Custom Modal States replacing unstyled alerts
  ...
  useEffect(() => {
    const fetchVoterData = async () => {
      try {
        ...
        // Fetch candidates for the voter's area
  ...
  const handleVoteConfirmation = (candidateId, candidateName) => {
    setModalConfig({
      ...
      description: `You are about to cast your digital franchise for ${candidateName}. This cryptographic action is legally final and cannot be altered or undone.`,
      candidateId,
      candidateName
    });
  };
  ...
  const executeVote = async () => {
    ...
    try {
      ...
      triggerAlert('success', 'Your electronic ballot ballot has been signed and recorded onto the secure ledger pool.');
      setVoted(true);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-[#F8FBFF] py-12 px-6 font-sans relative overflow-x-hidden select-none antialiased">
      ...
      {/* Decorative Blueprint Background Meshes */}
      ...
      <div className="relative z-10 max-w-4xl mx-auto space-y-8">
        ...
        {/* --- SECTION 1: CITIZEN MATRICES SUMMARY CARD --- */}
        {voter && (
          <div className="bg-white border border-slate-200/60 rounded-3xl p-6 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.02)] backdrop-blur-md flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-3.5">
              <div className="space-y-1">
                ...
                <h1 className="text-3xl font-black tracking-tight text-[#0F172A] font-display">{voter.name}</h1>
              </div>
              <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-[#64748B]">
                <span className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 border border-slate-200/60 rounded-xl">
                  <MapPin size={14} className="text-slate-400" /> Constituency: {voter.area}
                </span>
                <span className={`flex items-center gap-1.5 px-3 py-1 border rounded-xl font-bold uppercase tracking-wider text-[10px] ${
                  ...
                  {voted ? "Ballot Submitted" : "Ballot Pending Generation"}
                </span>
              </div>
            </div>
            
            {/* System Live Operational State Banner */}
            <div className={`px-4 py-3 rounded-2xl border text-center md:text-right shrink-0 ${
        ...
        <div className="space-y-4">
          <div className="border-b border-slate-200 pb-3">
            <h2 className="text-xl font-black text-[#0F172A] tracking-tight font-display">Candidates in Your Constituency</h2>
            <p className="text-xs font-medium text-[#64748B] mt-0.5">Please review official records and backgrounds carefully before choosing your node.</p>
          </div>
          ...
          {candidates.length === 0 ? (
            <div className="text-center py-12 bg-white border border-slate-200/60 rounded-2xl text-[#64748B] text-xs font-bold uppercase tracking-wider">
              No coordinating candidate indicators indexed for constituency: {voter?.area}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {candidates.map(candidate => (
                <div key={candidate._id} className="bg-white border border-slate-200/60 rounded-3xl p-6 shadow-[0_8px_30px_rgba(0,0,0,0.01)] backdrop-blur-md flex flex-col justify-between gap-6">
                  
                  {/* Top Block: Meta details & Badges */}
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wide mt-1">
                          <Gavel size={12} /> Legal Disclosures:
                          <span className={`font-black rounded px-1.5 py-0.5 uppercase text-[9px] ${
                            candidate.crimminalCase && candidate.crimminalCase.toLowerCase() !== 'none'
                              ? 'bg-amber-50 text-amber-800 border border-amber-100'
                              : 'bg-emerald-50 text-emerald-800 border border-emerald-100'
                          }`}>
                            {candidate.crimminalCase || 'None'}
                          </span>
                        </span>
                      </div>
                      
                      {/* Party Identity Stamp */}
                      {candidate.partyImage ? (
                        <img src={candidate.partyImage} alt="Party Emblem" className="w-10 h-10 object-contain bg-slate-50 border border-slate-100 p-1 rounded-xl shadow-xs" />
                      ) : (
                    ...
                    {/* Interactive Profile Media Thumbnail Wrap */}
                  ...
                  {/* Context Sensitive Interactive Action Buttons */}
                  {electionStatus.electionLive && !voted ? (
                    <button
                      onClick={() => handleVoteConfirmation(candidate._id, candidate.name)}
                      className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold text-xs rounded-xl py-2.5 shadow-sm shadow-blue-600/10 active:scale-[0.99] transition-all uppercase tracking-wider font-display cursor-pointer"
                    >
                      <Vote size={14} />
        ...
        {/* --- SECTION 3: SYSTEM AUDIT / CONCLUDED RESULTS ENTRYWAY --- */}
        {electionStatus.resultVisible && (
          <div className="pt-4 border-t border-slate-200">
            <button
              onClick={() => handleNavigate('/voter/results')}
              className="w-full inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-black text-white py-3.5 px-4 rounded-xl text-xs font-bold uppercase tracking-widest shadow-md hover:shadow-lg active:scale-[0.99] transition-all cursor-pointer font-display"
            >
              <BarChart3 size={15} />
          ...
          <div className="w-full max-w-md bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 shadow-[0_20px_50px_-12px_rgba(15,23,42,0.08)] space-y-6 text-center animate-[scaleUp_0.2s_ease-out]">
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={executeVote}
                className="w-1/2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-xs font-bold uppercase tracking-wider rounded-xl py-3 shadow-sm shadow-blue-600/10 transition-colors cursor-pointer"
              >
