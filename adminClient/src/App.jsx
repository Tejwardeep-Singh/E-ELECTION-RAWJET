...
function Results(){const [candidates,setCandidates]=useState([]),[message,setMessage]=useState(''); const { selectedElection } = useElection(); useEffect(()=>{if(!selectedElection){setCandidates([]);setMessage('');return;} setMessage('');api(`/api/results?role=head&electionId=${selectedElection._id}`, { role: 'head' }).then(setCandidates).catch(e=>setMessage(e.message))},[selectedElection]); if (!selectedElection) return <Page title="Results" subtitle="Election results and candidate vote counts."><Card className="text-center text-slate-500">Please select an election.</Card></Page>; const total=candidates.reduce((n,c)=>n+(c.voteCount||0),0),winner=candidates[0];return <Page title="Results" subtitle="Election results and candidate vote counts.">{message?<Card className="mt-5 text-center"><BarChart3 className="mx-auto mb-3 text-slate-300"/><p className="font-bold text-slate-700">Results are not available yet</p><p className="mt-1 text-sm text-slate-500">{message}</p></Card>:<><div className="mt-5 grid gap-4 sm:grid-cols-3"><Card><p className="text-xs font-bold uppercase text-slate-400">Winner</p><p className="mt-2 text-lg font-black">{winner?.name||'Awaiting votes'}</p></Card><Card><p className="text-xs font-bold uppercase text-slate-400">Total votes</p><p className="mt-2 text-lg font-black">{total}</p></Card><Card><p className="text-xs font-bold uppercase text-slate-400">Turnout</p><p className="mt-2 text-lg font-black">Available after finalisation</p></Card></div><Card className="mt-5"><h2 className="mb-4 font-bold">Candidate vote counts</h2>{candidates.map(c=><div className="mb-3 flex justify-between border-b pb-3 text-sm" key={c._id}><span>{c.name}</span><b>{c.voteCount||0} votes</b></div>)}</Card></>}</Page>}
